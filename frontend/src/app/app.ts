...
import { DashboardFilters, ImportPreview, Summary } from './models';
...
export class App {
  ...
  dashboardFilters:DashboardFilters=this.readDashboardFilters();
  ...
  loadDashboard(){this.loading.set(true);const params:Record<string,string|number>={};Object.entries(this.dashboardFilters).forEach(([key,value])=>{if(value!==undefined&&value!=='')params[key]=value;});this.http.get<Summary>(`${this.api}/api/v1/analytics/summary`,{...this.options(),params}).subscribe({next:v=>{this.summary.set(v);this.loading.set(false);},error:e=>this.fail(e,'The free API may be waking up. Please retry in about a minute.')});}
  updateDashboard(filters:DashboardFilters){this.dashboardFilters=filters;const url=new URL(location.href);['country','departmentId','jobLevelId','gender'].forEach(key=>url.searchParams.delete(key));Object.entries(filters).forEach(([key,value])=>{if(value!==undefined&&value!=='')url.searchParams.set(key,String(value));});history.replaceState({},'',url);this.loadDashboard();}
  ...
  private readDashboardFilters():DashboardFilters{const params=new URLSearchParams(location.search);const number=(key:string)=>{const value=params.get(key);return value?Number(value):undefined;};return{country:params.get('country')||undefined,departmentId:number('departmentId'),jobLevelId:number('jobLevelId'),gender:params.get('gender')||undefined};}
